#!/bin/bash
# ============================================
# Two-Stage Distributed Inference Evaluation Script
# Supports: Think Stage (large block) + Final Stage (small block)
...
CONFIG_FILE="${PROJECT_ROOT}/configs/eval_config/tdar_eval_tccf.yaml"  # TCCF config file

# --- Output Path Config ---
LOG_BASE_DIR="${PROJECT_ROOT}/logs_tccf"
RESULT_BASE_DIR="${PROJECT_ROOT}/results_tccf"
...
print_header "Two-Stage Distributed Inference Evaluation Script Started"
...
echo "🎯 Inference Mode: Two-Stage (Think + Final)"
echo "📋 Test Tasks: ${#DATASET_CONFIGS[@]}"
echo ""

# ============================================
# 🛠️ Single Dataset Two-Stage Evaluation Function
# ============================================
run_single_dataset_2stage() {
    local GPU_ID=$1
    ...
    local EXP_NAME="${EVAL_DATASET}_${RUN_ID}_2stage_${TIMESTAMP}"
    local LOG_FILE="${LOG_DIR}/${EXP_NAME}.log"
    
    print_info "[$INDEX] GPU-${GPU_ID} Starting: $EVAL_DATASET [$RUN_ID] (Two-Stage)"
    
    (
        export CUDA_VISIBLE_DEVICES=$GPU_ID
        
        echo "==================================="
        echo "🎯 Two-Stage Inference: $EVAL_DATASET [$RUN_ID]"
        echo "==================================="
        echo "Start Time: $(date '+%Y-%m-%d %H:%M:%S')"
        echo "GPU: $GPU_ID"
        echo "Mode: Think Stage (large block) + Final Stage (small block)"
        echo ""
        
        # 🎯 Core Command - Call two-stage evaluation script
        python "${PROJECT_ROOT}/src/eval_tccf.py" \
            config="${CONFIG_FILE}" \
            model="${MODEL_PATH}" \
        ...
        echo ""
        echo "==================================="
        [ $EXIT_CODE -eq 0 ] && echo "✅ Two-Stage Inference Success" || echo "❌ Two-Stage Inference Failed (code: $EXIT_CODE)"
...
# ============================================
print_header "Starting Two-Stage Parallel Evaluation"
...
while [ $BATCH_START -lt $TOTAL ]; do
    ...
    echo ""
    print_info "📦 Batch ${BATCH_NUM}: Launching ${BATCH_SIZE} two-stage tasks"
    ...
    for i in $(seq 0 $((BATCH_SIZE - 1))); do
        ...
        run_single_dataset_2stage "$GPU_ID" "$dataset" "$datatype" "$num_resp" "$run_id" "$((IDX + 1))" &
        PIDS+=($!)
    done
...
# 📊 Completion Report
# ============================================
print_header "Two-Stage Evaluation Complete"
...
print_success "All two-stage evaluation tasks completed!"
